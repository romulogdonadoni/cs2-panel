...
/** IDs de categoria e sub-filtro por `weapon.id` (API ByMykel / CS2). */
const CAT = {
...
export async function loadSkinsData(dataDir: string): Promise<SkinRow[]> {
  ...
  try {
    ...
    if (!r.ok) {
      throw new Error(`HTTP ${r.status} skins`);
    }
...
/**
 * Página da grelha: slice após filtro, para não enviar 20k+ items ao browser.
 */
